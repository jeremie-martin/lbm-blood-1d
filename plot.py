...
xxx = np.linspace(0, 6.0*3, len(y[0]) + len(y2[0]) + len(y3[0]))

fig, ax = plt.subplots(figsize=(14, 5))
# x = xxx[:int(len(xxx)*(6/15))]
# x2 = xxx[int(len(xxx) * (6 / 15)) :]
# print(x.shape, x2.shape)
x = np.linspace(0, 6.0*3, len(y[0]))
x2 = np.linspace(0, 6.0*3, len(y2[0]))
x3 = np.linspace(0, 6.0*3, len(y3[0]))
line, = ax.plot(x, np.sin(x / 100), 'b', label='A')
#  line2, = ax.plot(x2, np.sin(x2 / 100), 'r', label='B')
# line3, = ax.plot(x3, np.sin(x3 / 100), 'g', label='C')
plt.legend()
